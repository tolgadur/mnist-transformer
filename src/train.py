...
def train_transformer(epochs: int = 30, batch_size: int = 32, lr: float = 0.001):
    ...
    for epoch in range(epochs):
        ...
        for _, flattened_patches, input_seq, target_seq in tqdm.tqdm(dataloader):
            ...
            # Reshape logits and target for cross entropy loss
            # logits shape: (batch_size, seq_len_decoder, vocab_size)
            # target shape: (batch_size * seq_len_decoder, vocab_size)
            batch_size, seq_len, vocab_size = logits.shape
            logits = logits.reshape(-1, vocab_size)
            target_seq = target_seq.reshape(-1)
